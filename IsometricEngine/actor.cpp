...
//TODO, Change to Double- Currently there's a precision issues with framerate
void actor::update(float dt) {
	...
	if (!isOnGround && m_Zlevel > 0) vel.z -= 0.418f;
	
	//Friction- Currenctly losing .04% vectically, .13% horizontally
	if (abs(vel.length()) > 0) {
		if (isOnGround) {
			vel.y *= .89f;
			vel.x *= .89f;
