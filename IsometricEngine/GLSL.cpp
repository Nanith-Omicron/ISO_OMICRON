...
#include "IOManager.h"
	...
	void GLSLProgram::compileShadersFromSource(const char* vertexSource, const char* fragmentSource) {
		...
		if (m_programID == GL_FALSE) {
			printf("Program ID was not created!");

		}

		m_vertexShaderID = glCreateShader(GL_VERTEX_SHADER);
		if (m_vertexShaderID == GL_FALSE) {
			printf("Vertex Shader ID was not created!");
		}

		m_fragmentShaderID = glCreateShader(GL_FRAGMENT_SHADER);
		if (m_fragmentShaderID == GL_FALSE) {
			printf("Fragment Shader ID was not created!");
		}
	...
	void GLSLProgram::linkShaders() {
		...
		if (isLinked == GL_FALSE) {
			...
			printf("%s\n", &errorLog[0]);
			printf("Shaders were not linked to program!");
		}
	...
	GLuint GLSLProgram::getUniformLocation(const std::string& uniformName) {
		...
		if (location == GL_INVALID_INDEX) {
			printf("Uniform  not found in shader!");
		}
		return location;
	}
